// StranicaLige.tsx — sadržaj stranice lige (tablica, kola, utakmice,
// bočni stupac). Do 25.09.2026. je sve ovo stajalo u
// app/liga/[slug]/page.tsx i čitalo kolo i sezonu iz upita (?kolo=6), pa
// se stranica renderirala pri svakom otvaranju. Sada je kolo dio adrese.
...
/**
 * Cijela stranica lige. Koriste je četiri rute u app/liga/[slug]/, koje
 * se razlikuju samo po tome je li sezona i kolo zadano u adresi (vidi
 * adresaLige u lib/lige.ts). Nijedna ne čita parametre iz upita, pa se
 * sve mogu pripremiti unaprijed i držati kao gotove stranice.
 *
 * sezona  "2026/27"; bez nje tekuća, dakle najnovija sezona te lige
 * kolo    bez njega zadnje ODIGRANO kolo te sezone
 */
export default async function StranicaLige({
  ...
  // Adresa sa sezonom ili kolom kojih nema ne smije postati prazna
  // stranica: svaka takva bi se spremila kao zasebna gotova stranica.
  if (sezonaIzUrl && !sveSezone.includes(sezonaIzUrl)) notFound();
  if (koloIzUrl && !svaKola.includes(koloIzUrl)) notFound();

  const odabranoKolo = koloIzUrl ?? zadnjeOdigranoKolo ?? svaKola[0];
            ...
            <div className="flex gap-1">
              {sveSezone.map((s) => {
                ...
                return (
                  <Link
                    key={s}
                    href={adresaLige(slug, { sezona: s === tekucaSezona ? null : s })}
                ...
                <div className="mb-10 flex flex-wrap gap-1.5 pb-6" style={{ borderBottom: "1px solid var(--line)" }}>
                  {svaKola.map((k) => {
                    ...
                    return (
                      <Link
                        key={k}
                        href={adresaLige(slug, { kolo: k, sezona: sezonaZaAdresu })}
...
/**
 * Naslov, opis i adresa (og:url) stranice lige. Bez ovoga je svaka
 * stranica lige nosila generički naslov "Lokal-Arena" i opis cijele
 * stranice, a Facebookov provjeravač traži i og:url.
 */
export function metapodaciLige(
  slug: string,
  { sezona, kolo }: { sezona?: string; kolo?: number } = {}
): Metadata {
  const liga = ligaPoSlugu(slug);
  if (!liga) return { title: "Natjecanje nije pronađeno" };

  const dodatak = [kolo ? `${kolo}. kolo` : null, sezona ? `sezona ${sezona}` : null]
    .filter(Boolean)
    .join(", ");
  const naslov = dodatak
    ? `${liga.naziv}, ${dodatak}: rezultati i tablica`
    : `${liga.naziv}: rezultati, tablica i strijelci`;
  const opis = `Rezultati, raspored, tablica i strijelci natjecanja ${liga.naziv}${
    dodatak ? ` (${dodatak})` : ""
  }. Podaci s HNS Semafora, osvježeni svakog vikenda.`;
  const adresa = adresaLige(slug, { kolo, sezona });
...
/**
 * Parametri iz putanje u oblik koji StranicaLige prima. Neispravna
 * sezona ili kolo (npr. /kolo/abc) vodi na "nije pronađeno".
 */
export function parametriLige(p: { slug: string; sezona?: string; kolo?: string }): {
  slug: string;
  sezona?: string;
  kolo?: number;
} {
  const sezona = p.sezona !== undefined ? sezonaIzAdrese(p.sezona) : undefined;
  const kolo = p.kolo !== undefined ? koloIzAdrese(p.kolo) : undefined;
  if (sezona === null || kolo === null) notFound();
  return { slug: p.slug, sezona: sezona ?? undefined, kolo: kolo ?? undefined };
}
